...
import { getHttpConsoleLoggingFlag } from '../process-args';
...
const logHttp = getHttpConsoleLoggingFlag();
...
function attachRequestReaders(
	...
	const response$ = fromEvent<IncomingMessage>(clReq, 'response')
	.pipe(
		map(res => ({res, resBodyPromise: readAllBytesFrom(res)})),
		take(1)
	);
