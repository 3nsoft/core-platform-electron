...
export class CorrespondentKeys {
	...
	constructor(
			...
			address: string, serialData: string) {
		if (address) {
			this.keys = {
	 ...
	 */
	suggestPair(invitation: string): SuggestedNextKeyPair {
		let nextKeyPair: SuggestedNextKeyPair;
		if (this.keys.receptionPairs.suggested) {
			let suggPair = this.keys.receptionPairs.suggested;
			nextKeyPair = {
				pids: suggPair.pids,
	 ...
	 */
	getSendingPair(corrIntroKey: JsonKey = null): SendingPair {
		if (this.keys.sendingPair) { return this.keys.sendingPair; }
		let senderKey = util.generateKeyPair();
