...
export let specs: SpecDescribe = {
	description: '.versionedGetByteSink',
	its: []
};
...
it.func = async function(done: Function) {
	try {
		await testFS.versionedGetByteSink('non-existing-file', false)
		.then(() => {
...
it.func = async function(done: Function) {
	try {
		...
		let { sink, version: v } = await testFS.versionedGetByteSink(path);
...
it.func = async function(done: Function) {
	try {
		...
		let { sink } = await testFS.versionedGetByteSink(path);
...
it.func = async function(done: Function) {
	try {
		...
		let v = await testFS.versionedWriteBytes(path, bytes);
		cExpect(await testFS.checkFilePresence(path)).toBe(true);

		let { sink, version } = await testFS.versionedGetByteSink(path);
