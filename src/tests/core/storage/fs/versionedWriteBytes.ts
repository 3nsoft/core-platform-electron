...
export let specs: SpecDescribe = {
	description: '.versionedWriteBytes',
	its: []
};
...
it.func = async function(done: Function) {
	try {
		await testFS.versionedWriteBytes('non-existing-file', testRandomBytes(123), false)
		.then(() => {
...
it.func = async function(done: Function) {
	try {
		...
		let v = await testFS.versionedWriteBytes(path, content);
		cExpect(await testFS.checkFilePresence(path)).toBe(true);
		let { bytes, version } = await testFS.versionedReadBytes(path);
...
it.func = async function(done: Function) {
	try {
		...
		let v1 = await testFS.versionedWriteBytes(path, initBytes);
		cExpect(await testFS.checkFilePresence(path)).toBe(true);
		let { bytes, version } = await testFS.versionedReadBytes(path);
		...
		let v2 = await testFS.versionedWriteBytes(path, newContent);
		cExpect(await testFS.checkFilePresence(path)).toBe(true);
		cExpect(v2).toBeGreaterThan(v1);
		({ bytes, version } = await testFS.versionedReadBytes(path));
...
it.func = async function(done: Function) {
	try {
		...
		// setup initial file
		let initBytes = testRandomBytes(123);
		let v = await testFS.versionedWriteBytes(path, initBytes);
		cExpect(await testFS.checkFilePresence(path)).toBe(true);
		// try an exclusive write
		let newContent = testRandomBytes(3223);
		await testFS.versionedWriteBytes(path, newContent, true, true)
		.then(() => {
