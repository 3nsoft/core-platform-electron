...
export let specs: SpecDescribe = {
	description: '.versionedReadTxtFile',
	its: []
};
...
it.func = async function(done: Function) {
	try {
		...
		await testFS.versionedReadTxtFile(fName)
		.then(() => {
...
it.func = async function(done: Function) {
	try {
		...
		let fName = 'file1';
		let v1 = await testFS.versionedWriteTxtFile(fName, original);
		let { txt, version } = await testFS.versionedReadTxtFile(fName);
		...
		({ txt, version } = await testFS.versionedReadTxtFile(fName));
