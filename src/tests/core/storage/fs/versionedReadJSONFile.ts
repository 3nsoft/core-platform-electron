...
export let specs: SpecDescribe = {
	description: '.versionedReadJSONFile',
	its: []
};
...
it.func = async function(done: Function) {
	try {
		...
		await testFS.versionedReadJSONFile(fName)
		.then(() => {
...
it.func = async function(done: Function) {
	try {
		...
		let fName = 'file1';
		let v1 = await testFS.versionedWriteJSONFile(fName, original);
		let { json, version } = await testFS.versionedReadJSONFile(fName);
