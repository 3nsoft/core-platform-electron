...
export let specs: SpecDescribe = {
	description: '.versionedWriteJSONFile',
	its: []
};
...
it.func = async function(done: Function) {
	try {
		let json = { a: 1, b: 2 };
		await testFS.versionedWriteJSONFile('non-existing-file', json, false)
		.then(() => {
...
it.func = async function(done: Function) {
	try {
		...
		let v = await testFS.versionedWriteJSONFile(path, initJson);
		cExpect(await testFS.checkFilePresence(path)).toBe(true);
		let { json, version } = await testFS.versionedReadJSONFile(path);
...
it.func = async function(done: Function) {
	try {
		...
		// setup initial file
		let initJson = { a: 'foo', b: true, 'df-df': 23};
		let v1 = await testFS.versionedWriteJSONFile(path, initJson);
		cExpect(await testFS.checkFilePresence(path)).toBe(true);
		let { json, version } = await testFS.versionedReadJSONFile(path);
		cExpect(deepEqual(initJson, json)).toBe(true);
		...
		let newJson = null;
		let v2 = await testFS.versionedWriteJSONFile(path, newJson);
		cExpect(await testFS.checkFilePresence(path)).toBe(true);
		({ json, version } = await testFS.versionedReadJSONFile(path));
...
it.func = async function(done: Function) {
	try {
		...
		let newJson = null;
		await testFS.versionedWriteJSONFile(path, newJson, true, true)
		.then(() => {
