...
it.func = async function(done: Function) {
	try {
		...
		cExpect(bytes.length).toBe(0, 'reading empty file should produce empty array');
	} catch (err) {
		cFail(err);
	}
	done(collectAllExpectations());
...
it.func = async function(done: Function) {
	try {
		...
		bytes = await testFS.readBytes(fName, null, 123);
		...
		cExpect(bytes.length).toBe(0, 'when start is greater than file size, empty array must be returned');
		...
		cExpect(bytes.length).toBe(0, 'when end is smaller than start , empty array must be returned');
