...
it.func = async function(done: Function) {
	try {
		...
		cExpect(bytesEqual(bytes, await testFS.readBytes(path))).toBe(true);
	} catch (err) {
		cFail(err);
	}
	done(collectAllExpectations());
...
it.func = async function(done: Function) {
	try {
		...
		cExpect(await testFS.checkFilePresence(path)).toBe(true);
		cExpect(bytesEqual(initBytes, await testFS.readBytes(path))).toBe(true);
		// write new file content
		let newContent = testRandomBytes(3223);
		await testFS.writeBytes(path, newContent);
		cExpect(await testFS.checkFilePresence(path)).toBe(true);
		cExpect(bytesEqual(newContent, await testFS.readBytes(path))).toBe(true);
	} catch (err) {
		cFail(err);
	}
	done(collectAllExpectations());
...
it.func = async function(done: Function) {
	try {
		...
		let bytes = testRandomBytes(3223);
		await testFS.writeBytes(path, bytes, true, true)
		.then(() => {
			cFail('exclusive-create write operation must fail, when file exists.');
		}, (exc: FileException) => {
			cExpect(exc.alreadyExists).toBe(true);
		});
		cExpect(bytesEqual(initBytes, await testFS.readBytes(path))).toBe(true, 'initial file content stays intact');
	} catch (err) {
		cFail(err);
	}
	done(collectAllExpectations());
