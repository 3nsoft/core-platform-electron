...
class CachedByteSource implements ByteSource {
	...
	private baseSrc: ByteSource = null;
	...
	private async getChunk(start, end): Promise<Uint8Array> {
	...
	private async nonDiffRead(len: number): Promise<Uint8Array> {
		let start = this.segsPointer;
		let end = ((typeof len === 'number') ? (start + len) : this.segsSize);
		return this.getChunk(start, end);
	}

	private async diffRead(len: number): Promise<Uint8Array> {
		let start = this.segsPointer;
		...
		if (typeof fstSecInd !== 'number') { return null; }
		...
		for (let i=fstSecInd; i <= lastSecInd; i+=1) {
			...
			if (s[0] === 0) {
				...
				baseSrc.seek(s[1]);
				buf.push(await baseSrc.read(s[2]));
			} else {
				buf.push(await this.getChunk(s[1], s[1]+s[2]));
			}
		}
		return buf.getBytes(null);
	}
...
/**
 * @param cache
 * @param downloader is required when there is no 100% guarantee that all bytes
 * are in cache already. When all bytes are in cache, null can be given.
 * @param objId
 * @param version
