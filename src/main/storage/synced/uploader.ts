...
import { SyncAction, SyncLog, CacheFiles, SyncLogs } from './cache-files';
...
export class Uploader {
	...
	constructor(
			...
			private syncLogs: SyncLogs,
			private remoteStorage: RemoteStorageOwner) {
		Object.seal(this);
	}

	private async updateLog(objId: string,
			update: (log: SyncLog|undefined) => SyncLog): Promise<void> {
		return this.logProcs.startOrChain(objId, async () => {
			let log = await this.syncLogs.get(objId);
			log = update(log);
			await this.syncLogs.set(objId, log);
		});
	}
	...
	async activateSyncAction(objId: string, actNum: number): Promise<void> {
		await this.updateLog(objId, (log) => {
			if (!log) { throw new Error(
				`Sync log is missing on action activation for object ${objId}`); }
	...
	private startSync(objId: string, recursiveStart = false): void {
		this.logProcs.startOrChain(objId, async () => {
			...
			let log = await this.syncLogs.get(objId);
			if (!log) { throw new Error(
				`Sync log is missing on action start for object ${objId}`); }
			...
			if (log.currentAction) {
				...
				await this.syncLogs.clear(objId);
				this.files.gc.scheduleCollection(objId);
				return;
			}
	...
	private async logProgress<T>(objId: string, actionProgress: T):
		...
		await this.updateLog(objId, (log) => {
			if (!log) { throw new Error(
				`Sync log is missing on logging progress for object ${objId}`); }
	...
	private async completeCurrentAction(objId: string): Promise<void> {
		await this.updateLog(objId, (log) => {
			if (!log) { throw new Error(
				`Sync log is missing on action completion for object ${objId}`); }
			log.progress = undefined;
			log.currentAction = (undefined as any);
			return log;
		});
		this.files.gc.scheduleCollection(objId);
	}
